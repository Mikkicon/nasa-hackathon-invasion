...
def read_gdf(name, geocols=[]):
  print('read_gdf', name)
  try:
    _df = pd.read_csv(name)
    for col in geocols:
      _df[col] = _df[col].apply(wkt.loads)
    _df = gpd.GeoDataFrame(_df, geometry='geometry')
    _df.set_crs(epsg=4326, inplace=True) 
    return _df
  except Exception as e:
    print("read_gdf error: ", e)  
    return None
...
def create_prediction_map(map_data, gdf):
    try:
        center_lat, center_lon = map_data["center"]
        m = folium.Map(location=[center_lat, center_lon], zoom_start=7)
        # Add layer control
        ...
        road_cluster = MarkerCluster(name="Road").add_to(m)
        for _, row in gdf.iterrows():
            folium.Marker(
                location=[row.geometry.y, row.geometry.x],
...
def main():
    ...
    results_gdf = read_gdf("result_df_border_bboxes.csv", geocols=['geometry', 'nearest_road'])
    print('results_gdf', results_gdf.shape, 'pred_map' in st.session_state)
